...
Since virtual clusters share physical nodes with the host cluster, Rancher may count virtual cluster nodes toward your node count or resource usage. To prevent this, use sync patches to zero out the CPU capacity reported by the virtual cluster's nodes.

:::info Enterprise feature
Node sync patches require a vCluster Pro or Enterprise license.
:::
...
:::note
This approach requires `sync.fromHost.nodes.enabled: true` and is not compatible with the [virtual scheduler](../../configure/vcluster-yaml/control-plane/other/advanced/virtual-scheduler.mdx), which relies on accurate node capacity information for scheduling decisions.
:::
